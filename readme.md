...
这是一个简洁美观的超轻量化的准静态个人博客网页，采用左右布局设计：左侧导航栏，右侧内容区域。
内容创建简单，仅需要将文本TXT文件或者MarkDown文件放入context下对应的文件夹，即可实现内容增加。如果删除文件，网页也能自动删除。

DEMO页面  https://hongyunqiu.github.io/DrQiuBlog/
...
#### 1. **文件扫描机制**
- 使用 Python/Node.js 脚本扫描 `context/` 目录
- 自动发现所有 `.txt` 和 `.md` 文件
- 实时检测文件的变化（增加、删除、修改）
...
- 脚本会读取每个文件的前几行作为描述
- 自动生成唯一的文件ID和配置信息
- 完全重建 `config.js` 配置文件，确保与实际文件同步

#### 3. **智能更新策略**
- **添加文件**：扫描时自动发现新文件并添加到配置
- **删除文件**：重新扫描时自动从配置中移除已删除的文件
- **修改文件**：检测文件修改时间，更新最后修改时间戳

#### 4. **多种更新方式**
- **手动更新**：运行 `update-config-py.bat` 一键更新配置
- **自动监控**：使用 Node.js 监控文件变化，实时自动更新
- **启动时更新**：每次启动服务器时自动扫描并更新配置

#### 5. **安全保护机制**
- 每次更新前自动备份原配置文件到 `config.js.backup`
- 支持配置回滚，避免数据丢失
- 错误处理和异常捕获，确保系统稳定性
